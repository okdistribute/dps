...
module.exports = function (resource, cb) {
  ...
  if (!resource) return cb(new Error('Resource undefined'))
...
function fetchDat (resource, cb) {
  datPing(resource.location, function (err, status) {
    ...
    resource.name = resource.name || status.dat.name
    resource.description = resource.description || status.dat.description
    resource.publisher = resource.publisher || status.dat.publisher
