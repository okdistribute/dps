...
module.exports = function download (source, cb) {
  ...
  fetch(source, function (err, source) {
    ...
    var last_modified = new Date(old.meta.modified).toString()
    if (old.meta && last_modified === source.meta.modified.toString()
        && old.meta.size === source.meta.size) return cb(null, source)
